...
class LevelSession:
    def __init__(self, bot):
        ...
        self.message_experience = 5
        self.player_message_cooldowns = {}

        self.reaction_cooldown = 30
        self.reaction_experience = 5
        self.player_reaction_cooldowns = {}

    def give_message_experience(self, member: discord.Member):
        """
        ...
        # 檢查冷卻時間
        if user_id in self.player_message_cooldowns:
            if now - self.player_message_cooldowns[user_id] < self.message_cooldown:
                return
        
        # 更新冷卻時間戳
        self.player_message_cooldowns[user_id] = now

        Player.add_experience(user_id, self.message_experience)

        logger.debug(f"給予 {member.display_name} 訊息經驗值: {self.message_experience} exp")
    ...
    def give_reaction_experience(self, member: discord.Member):
        ...
        # 檢查冷卻時間
        if user_id in self.player_reaction_cooldowns:
            if now - self.player_reaction_cooldowns[user_id] < self.reaction_cooldown:
                return
        
        # 更新冷卻時間戳
        self.player_reaction_cooldowns[user_id] = now

        Player.add_experience(user_id, self.reaction_experience)

        logger.debug(f"給予 {member.display_name} 反應經驗值: {self.reaction_experience} exp")
